          ...
          withParameters:(NSArray*)params 
{
	...
	for (NSUInteger i = 0; i < [params count]; ++i) 
	{
		...
		{
			SEL returnSixtyFour = @selector(sheetDidEnd:returnCode64:contextInfo:);
			SEL returnThirtyTwo = @selector(sheetDidEnd:returnCode32:contextInfo:);
			SEL returnEight = @selector(sheetDidEnd:returnCode8:contextInfo:);
			
			if (info->arg1OfEndSelectorSize_ == 64)
				[invocation setArgument:&returnSixtyFour
								atIndex:i+2];
			else if (info->arg1OfEndSelectorSize_ == 32)
				[invocation setArgument:&returnThirtyTwo
								atIndex:i+2];
			else if (info->arg1OfEndSelectorSize_ == 8)
				[invocation setArgument:&returnEight
								atIndex:i+2];
		} 
