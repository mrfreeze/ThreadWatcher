...
- (void)startRequest
{
...
#endif
#else
			NSDictionary *proxySettings = [(NSDictionary *)CFMakeCollectable(SCDynamicStoreCopyProxies(NULL)) autorelease];
#endif
			
			proxies = [(NSArray *)CFMakeCollectable(CFNetworkCopyProxiesForURL((CFURLRef)[self url], (CFDictionaryRef)proxySettings)) autorelease];
	...
	if (shouldAttemptPersistentConnection) {
		...
		if ([self connectionInfo]) {
			...
			if (![[[self connectionInfo] objectForKey:@"host"] isEqualToString:[[self url] host]] || 
				![[[self connectionInfo] objectForKey:@"scheme"] isEqualToString:[[self url] scheme]] || 
				[(NSNumber *)[[self connectionInfo] objectForKey:@"port"] intValue] != [[[self url] port] intValue]) 
			{
				[self setConnectionInfo:nil];
		...
		if (![self connectionInfo] && [[self url] host] && [[self url] scheme]) 
		{
			...
			for (i=0; i<[persistentConnectionsPool count]; i++) 
			{
				...
				if ([[existingConnection objectForKey:@"host"] isEqualToString:[[self url] host]] 
					&& [[existingConnection objectForKey:@"scheme"] isEqualToString:[[self url] scheme]] && 
					[(NSNumber *)[existingConnection objectForKey:@"port"] intValue] == [[[self url] port] intValue] &&
					![existingConnection objectForKey:@"request"]) 
				{
...
- (void)checkRequestStatus
{
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
			...
			[self setTotalBytesSent:[[(NSNumber *)CFMakeCollectable(CFReadStreamCopyProperty((CFReadStreamRef)[self readStream], kCFStreamPropertyHTTPRequestBytesWrittenCount)) autorelease] unsignedLongLongValue]];
	...
	if ([self responseHeaders] && ([self contentLength] || [self complete])) {
		...
		if (downloadProgressDelegate) {
			...
			if ([self contentLength]) {
				...
				{
					progress = (double)((1.0*bytesReadSoFar)/(double)(contentLength+partialDownloadSize));
				}
			}
			[self setUpdatedProgress:YES];
	...
	if (CFHTTPMessageIsHeaderComplete(headers)) {
		...
		if (![self authenticationNeeded]) {
			...
			if (cLength) {
				...
				if ([self mainRequest] && [self allowCompressedResponse] && length == 20 && 
					[self showAccurateProgress] && [self shouldResetProgressIndicators]) 
				{
...
- (BOOL)applyProxyCredentials:(NSDictionary *)newCredentials
{
	...
	if (newCredentials && proxyAuthentication && request) {
		...
		if (CFHTTPMessageApplyCredentialDictionary(request, proxyAuthentication, (CFMutableDictionaryRef)newCredentials, NULL)) {
			...
			[self setProxyCredentials:[NSMutableDictionary dictionaryWithDictionary:newCredentials]];
			return YES;
		} else {
...
- (BOOL)applyCredentials:(NSDictionary *)newCredentials
{
	...
	if (newCredentials && requestAuthentication && request) {
		...
		if (CFHTTPMessageApplyCredentialDictionary(request, requestAuthentication, (CFMutableDictionaryRef)newCredentials, NULL)) {
			...
			[self setRequestCredentials:[NSMutableDictionary dictionaryWithDictionary:newCredentials]];
			return YES;
		} else {
...
{	
	...
	/* *POSSIBLE HACK*
	 now that it uses persistent connections, it wasn't reading the response headers till
	 entire files were downloaded, so we need to make sure it checks for headers
	 with every network event, till they are downloaded. might slow connections down a bit but probably not, 
	 the header should arrive quick and after that it's just a single check to see if responseHeaders exist*/
	// 7 Jan, 2010, Mr Freeze
...
{
	NSError *underlyingError = [(NSError *)CFMakeCollectable(CFReadStreamCopyError((CFReadStreamRef)[self readStream])) autorelease];
...
+ (NSString *)defaultUserAgentString
{
	...
	if ((Gestalt(gestaltSystemVersionMajor, &versionMajor)) != noErr) return nil;
	if ((Gestalt(gestaltSystemVersionMinor, &versionMinor)) != noErr) return nil;
	if ((Gestalt(gestaltSystemVersionBugFix, &versionBugFix)) != noErr) return nil;
...
+ (NSArray *)proxiesForURL:(NSURL *)theURL fromPAC:(NSURL *)pacScriptURL
{
	...
	NSArray *proxies = [(NSArray *)CFMakeCollectable(CFNetworkCopyProxiesForAutoConfigurationScript((CFStringRef)script,(CFURLRef)theURL, &err2)) autorelease];
	if (err2) {
		return nil;
	}
#endif
