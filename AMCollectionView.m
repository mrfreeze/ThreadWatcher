...
- (void)awakeFromNib
{
	...
	if ([self itemPrototype]) 
	{
		...
		am_itemRespondsToSizeForViewWithProposedSize = 
							[[self itemPrototype] respondsToSelector:@selector(sizeForViewWithProposedSize:)];
...
- (void)viewDidEndLiveResize
{
	...
	[self resizeSubviewsWithOldSize:previousSize];
...
- (void)setItemPrototype:(AMCollectionViewItem *)value
{
	...
	else 
	{
		if (itemPrototype != value) 
		{
			...
			am_itemRespondsToSizeForViewWithProposedSize = 
						[itemPrototype respondsToSelector:@selector(sizeForViewWithProposedSize:)];
	...
	if (content != value) {
		...
		while ((object = [enumerator nextObject])) 
		{
			...
			else 
			{
				...
				am_itemRespondsToSizeForViewWithProposedSize = (am_itemRespondsToSizeForViewWithProposedSize || 
																[item respondsToSelector:@selector(sizeForViewWithProposedSize:)]);
			}
			...
			if (item) 
			{
				...
				if (NSIntersectsRect(rect, itemFrame)) 
				{
					...
					else 
					{
						...
						if (colorIndex != 0)
						{ // we already painted the whole thing in this color
...
{	
	...
	if (am_itemRespondsToSizeForViewWithProposedSize && didChangeWidth && !resizing) 
	{
		[self setNeedsLayout:YES];
		[self setNeedsDisplay:YES];
	}
